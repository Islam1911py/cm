...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const ticketNumberParam = searchParams.get("ticketNumber") || searchParams.get("ticket_number") || searchParams.get("number")
    ...
    // استعلام عن تذكرة واحدة برقم الشكوى (مثلاً TICK-ABC12345)
    if (ticketNumberParam && typeof ticketNumberParam === "string") {
        ...
        return NextResponse.json(
          {
            ...
            humanReadable: {
              ar: `مفيش شكوى برقم ${ticketNumberParam} مرتبطة برقمك، أو الرقم غير صحيح.`
            }
          },
          { status: 404 }
        )
      ...
      return NextResponse.json({
        ...
        humanReadable: {
          ar: `التذكرة ${ticketNumber} حالياً ${statusLabel[owned.status] ?? owned.status}.`
        }
      })
    }

    // قائمة التذاكر (السلوك الأصلي)
